...
public class SelectUnits : MonoBehaviour {
    ...
    static Texture2D _SelectionDraw;
    private static Texture2D SelectionDraw
    {
        get
        {
            if (_SelectionDraw == null)
            {
                _SelectionDraw = new Texture2D(1, 1);
                _SelectionDraw.SetPixel(0, 0, Color.white);
                _SelectionDraw.Apply();
            }

            return _SelectionDraw;
        }
    }
    ...
    void Update () { 
        ...
        { 
            ...
            if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hit))
            {
                ...
                if (hit.transform.gameObject.layer == LayerMask.NameToLayer("MetalLayer"))
                {
                    // Set destination interactable zone
                    _selectedTarget = hit.transform.gameObject;
                    BroadcastNewTarget(Constants.TargetType.MetalZone);
                    return;
                }
            }
        }
